      ...
      if ($institute->update(['status' => $status])) {
        if ($status === Institute::$statusArrays[1]) {
          try {
            ...
            \Illuminate\Support\Facades\Mail::to($institute->email)->cc([$user->email])->send(new \App\Mail\InstituteActivationMail($institute->name, $institute->code, $user->name_en, 'active'));
          } catch (\Exception $e) {
          }
          ...
          try {
            ini_set('max_execution_time', 300);
            \Illuminate\Support\Facades\Mail::to($institute->email)->cc([$user->email])->send(new \App\Mail\InstituteActivationMail($institute->name, $institute->code, $user->name_en, 'inactive'));
          } catch (\Exception $e) {
          }
        }
        return "success";
