...
//    return $request;
    $message = '<strong>Congratulations!!!</strong> Graduate/Student successfully ';
...
//    $rules['s_session'] = 'nullable|string';
    ...
    try {
      ...
      if ($request->hasFile('profile_photo_path')) {
        $image = CustomHelper::storeImage($request->file('profile_photo_path'), '/image/');
        if ($image != false) {
          $user->image = $image;
        }
      }
      ...
      if ($user->save()) {
        ...
        if ($old_image != $user->image) {
          CustomHelper::deleteFile($old_image);
        }
