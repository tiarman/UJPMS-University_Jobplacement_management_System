                ...
                @foreach($fairlist as $key => $val)
                 ...
                 <tr class="@if(($key%2) == 0)gradeX @else gradeC @endif">
                   ...
                   <td class="p-1">{{ $val->jobEvent?->place }}</td>
                                            <td class="p-1 text-capitalize">@if ($val->jobEvent)
                                                {{ $val->status }}
                                            @endif</td>

                    @if(\App\Helper\CustomHelper::canView('Manage Student Post|Delete Student Post', 'Student'))
                      <td class="text-center p-1" width="100">
                        @if(\App\Helper\CustomHelper::canView('Manage Student Post', 'Student'))

                          <a href="{{ route('admin.event.job.manage',$val->jobEvent?->id) }}"
                             class="btn btn-sm btn-primary"> <i class="fa fa-cogs"></i> </a>
                        @endif
                        {{--  @if(\App\Helper\CustomHelper::canView('Delete Student Post', 'Student'))
