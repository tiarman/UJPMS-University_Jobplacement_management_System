        ...
        <div class="card-body">
          <section class="panel">
            <header class="panel-heading">
              <h2 class="panel-title">JobPost Tag Type</h2>
            </header>
                ...
                <div class="row">
                  <div class="col-sm-6">
                    <div class="form-group">
                      <label class="control-label">Job Post Tag Type<span class="text-danger">*</span></label>
                  ...
                  <div class="col-sm-6">
                    <div class="form-group">
                      ...
                      <select name="status" required class="form-control @error('status') is-invalid @enderror">
                        ...
                        @foreach(\App\Models\Tag::$statusArrays as $status)
                          <option value="{{ $status }}"
    ...
    <div class="col-12">
      <div class="card">
        <div class="card-body">
          <section class="panel">
            <header class="panel-heading">
              <h2 class="panel-title">Job Post Tag Type List</h2>
            </header>
            <div class="panel-body">
              @if(session()->has('status'))
                {!! session()->get('status') !!}
              @endif
              @if(\App\Helper\CustomHelper::canView('Create Tag', 'Super Admin'))
              ...
              <table id="datatable-buttons" class="table table-striped table-bordered dt-responsive nowrap"
                ...
                <tr>
                  ...
                  <th>User_id</th>
                  <th>Status</th>
                  @if(\App\Helper\CustomHelper::canView('Manage Technology|Delete Technology', 'Super Admin'))
                ...
                @foreach($datas as $key => $val)
                  <tr class="@if(($key%2) == 0)gradeX @else gradeC @endif">
                    ...
                    <td class="p-1 text-capitalize">{{ $val->user_id }}</td>

                    @if(\App\Helper\CustomHelper::canView('', 'Super Admin'))
                          <td class="text-capitalize p-1" width="100">
                              <div class="onoffswitch">
                                  <input type="checkbox" name="onoffswitch" class="onoffswitch-checkbox"
                                         @checked($val->status == \App\Models\Tag::$statusArrays[1])
                                         data-id="{{ $val->id }}"
                    ...
                    @if(\App\Helper\CustomHelper::canView('', 'Super Admin'))
{{--                    @if(\App\Helper\CustomHelper::canView('Manage Tag|Delete Tag', 'Super Admin'))--}}
                      <td class="center hidden-phone p-1" width="100">
                          @if(\App\Helper\CustomHelper::canView('Manage Tag', 'Super Admin'))
                          ...
                          @endif
                          @if(\App\Helper\CustomHelper::canView('Delete Tag', 'Super Admin'))
  ...
  <script>
    $(document).ready(function () {
      ...
      $(document).on('change', 'input[name="onoffswitch"]', function () {
        ...
        $.ajax({
          url: "{{ route('admin.ajax.update.tag.status') }}",
          method: "post",
          dataType: "html",
