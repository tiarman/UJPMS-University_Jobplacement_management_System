        ...
        <div class="card-body">
          <section class="panel">
            <header class="panel-heading">
              <h2 class="panel-title">Create Department/Technology/Trade/Occupation</h2>
            </header>
            <div class="panel-body">
              @if(\App\Helper\CustomHelper::canView('Create Technology', 'Super Admin'))
              {{-- <div class="row">
                ...
                <div class="row">
                  <div class="col-sm-6">
                    <div class="form-group">
                      <label class="control-label">Department/ Technology/ Trade/ Occupation Name<span class="text-danger">*</span></label>
                  ...
                  <div class="col-sm-6">
                    <div class="form-group">
                      ...
                      <select name="status" required class="form-control @error('status') is-invalid @enderror">
                        ...
                        @foreach(\App\Models\Technology::$statusArrays as $status)
                          <option value="{{ $status }}"
    ...
    <div class="col-12">
      <div class="card">
        <div class="card-body">
          <section class="panel">
            <header class="panel-heading">
              <h2 class="panel-title">Department/Technology/Trade/Occupation List</h2>
            </header>
            <div class="panel-body">
              @if(session()->has('status'))
                {!! session()->get('status') !!}
              ...
              <table id="datatable-buttons" class="table table-striped table-bordered dt-responsive nowrap"
                ...
                <tr>
                  ...
                  <th>Updated By</th>
                  <th>Status</th>
                  @if(\App\Helper\CustomHelper::canView('Manage Technology|Delete Technology', 'Super Admin|Institute Head'))
                ...
                @foreach($datas as $key => $val)
                  <tr class="@if(($key%2) == 0)gradeX @else gradeC @endif">
                    ...
                    <td class="p-1 text-capitalize">{{ $val?->created_by_user->name_en }}</td>
                    <td class="p-1 text-capitalize">{{ $val->updated_by_user->name_en ?? "N/A" }}</td>

                    @if(\App\Helper\CustomHelper::canView('Manage Institute', 'Super Admin|Institute Head'))
                          <td class="text-capitalize p-1" width="100">
                              <div class="onoffswitch">
                                  <input type="checkbox" name="onoffswitch" class="onoffswitch-checkbox"
                                         @checked($val->status == \App\Models\Technology::$statusArrays[1])
                                         data-id="{{ $val->id }}"
                    ...
                    @if(\App\Helper\CustomHelper::canView('Manage Technology|Delete Technology', 'Super Admin|Institute Head'))
                      <td class="center hidden-phone p-1" width="100">
                          @if(\App\Helper\CustomHelper::canView('Manage Technology', 'Super Admin|Institute Head'))
                          ...
                          @endif
                          @if(\App\Helper\CustomHelper::canView('Delete Technology', 'Super Admin|Institute Head'))
  ...
  <div class="modal fade" id="manageTrainingTypeModal" tabindex="-1" role="dialog" aria-labelledby="manageTrainingType"
      ...
      <div class="modal-dialog modal-dialog-centered" role="document">
        <div class="modal-content">
            <header class="panel-heading">
              <h2 class="panel-title ml-4">Manage Technology</h2>
            </header>
          <div class="modal-body">
            <div class="panel-body">
              @if(\App\Helper\CustomHelper::canView('List Of Technology', 'Super Admin'))
              @endif
              @if(session()->has('status'))
                {!! session()->get('status') !!}
              @endif
  ...
  <div class="modal fade" id="userDeleteModal" tabindex="-1" role="dialog" aria-labelledby="userDeleteModal"
    ...
    <div class="modal-dialog modal-dialog-centered" role="document">
      <div class="modal-content">
        ...
        <div class="modal-body">
          <strong>Are you sure to delete this Technology?</strong>
        </div>
  ...
  <script>
    $(document).ready(function () {
      ...
      $(document).on('click', '.yes-btn', function () {
        ...
        $.ajax({
          url: "{{ route('admin.technology.destroy') }}",
          method: "DELETE",
          dataType: "html",
          data: {id: pid},
      ...
      $(document).on('change', 'input[name="onoffswitch"]', function () {
        ...
        $.ajax({
          url: "{{ route('admin.ajax.update.technology.status') }}",
          method: "post",
          dataType: "html",
