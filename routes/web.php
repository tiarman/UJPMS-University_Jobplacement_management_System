...
use App\Http\Controllers\AddPaymentInfoController;
use App\Http\Controllers\AdminController;
use App\Http\Controllers\AdminVoucherController;
use App\Http\Controllers\Api\ApiController;
use App\Http\Controllers\AppliedStudentAcceptanceByInstituteController;
...
use App\Http\Controllers\PurchaseMailController;
...
Route::prefix('/admin')->name('admin.')->middleware(['auth', 'verified'])->group(function () {
  ...
  Route::prefix('ajax')->name('ajax.')->group(function () {
    ...
    Route::post('/update/tag/status', [TagController::class, 'ajaxUpdateStatus'])->name('update.tag.status');
    ...
    Route::post('/update/instituteAcceptance/status', [AppliedStudentAcceptanceByInstituteController::class, 'ajaxUpdateStatus'])->name('update.instituteAcceptance.status');
  ...
  Route::prefix('event')->name('event.')->group(function () {
    Route::prefix('/job')->name('job.')->group(function () {
      ...
      Route::get('/manage/{id}', [JobEventController::class, 'manage'])->middleware('role_or_permission:Institute Head|Industry|Student|Manage Job Event')->name('manage');
      ...
      Route::get('/{id}/job-fair/applied/job-post-list', [GraduateController::class, 'jobFairJobPostAppliedList'])->middleware('role_or_permission:Student|List Of Job Event')->name('jobfair.applied.post.list');
    });
  ...
  Route::prefix('payment')->name('payment.')->group(function () {
    Route::get('/create', [PaymentController::class, 'create'])->middleware('role_or_permission:Super Admin')->name('create');
    Route::post('/store', [PaymentController::class, 'store'])->middleware('role_or_permission:Super Admin')->name('store');
    Route::get('/manage/{id}', [PaymentController::class, 'manage'])->middleware('role_or_permission:Super Admin')->name('manage');
    Route::get('/{id}/view', [PaymentController::class, 'view'])->middleware('role_or_permission:Super Admin|View Payment')->name('view');
    Route::delete('/destroy', [PaymentController::class, 'destroy'])->middleware('role_or_permission:Super Admin')->name('destroy');
    Route::get('/list', [PaymentController::class, 'index'])->middleware('role_or_permission:Super Admin|Institute Head|List Of Payment')->name('list');
  });
...
Route::get('/email_template', function () {
  return view('layout.email_template');
})->name('trial');

// Route::match(['get,post'],'/free-trial', [function(){
//   return view('layout.email_template');
// }])->name('free.trial');

Route::match(['get', 'post'], '/free-trial', [function (Request $request) {
  $name = $request->name;
  $email = $request->email;
  $phone = $request->phone;
  $mssg = "We appreciate your interest in JPMS trial version. We will send you an email with your login details (email and password) as soon as possible.";

  if ($request->isMethod('POST')) {
    //The email sending is done using to method on the Mail facade.
    Mail::to([env('APP_SEND_EMAIL', 'ceo@touchandsolve.com'), 'ceo@touchandsolve.com'])->send(new AdminNotifyForUserEmail($name, $email, $phone));

    return RedirectHelper::routeSuccess('trial', $mssg);
  }
  return view('layout.email_template');
}])->name('free.trial');

# job applied student acceptance by institute
Route::prefix('/applied-student')->name('applied.student.')->group(function () {
  Route::get('/list', [AppliedStudentAcceptanceByInstituteController::class, 'index'])->name('list');
});

Route::match(['get', 'post'], '/send-contact-mail', [function (Request $request) {
  $name = $request->name;
  $email = $request->email;
  $phone = $request->phone;
  ...
  if ($request->isMethod('POST')) {
    Mail::to(['ceo@touchandsolve.com'])->send(new \App\Mail\DemoMail($name, $email, $phone, $contact_message));
    return RedirectHelper::routeSuccess('send-contact-mail', $mess);
  }
  return view('site.ims_contact');
}])->name('send-contact-mail');


Route::match(['get', 'post'], '/send_purchase_mail', [PurchaseMailController::class, 'send_purchase_mail'])->name('send_purchase_mail');


//Route::get('/add_payment_info', function (){
